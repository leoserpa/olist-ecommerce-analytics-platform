...
OUTPUT_PARQUET = Path("data/olist_processado.parquet")
RECEITA_PARQUET = Path("data/olist_receita.parquet")
COMPLETO_PARQUET = Path("data/olist_completo.parquet")
OUTPUT_REPORT = Path("data/relatorio_qualidade.html")
LOG_DIR = Path("logs")
...
def etapa_salvar_e_validar(df: pd.DataFrame, log: logging.Logger):
    """Salva o Parquet e executa validações rigorosas."""
    log.info("💾 [8/8] Salvando Parquet e validando...")

    OUTPUT_PARQUET.parent.mkdir(parents=True, exist_ok=True)
    df.to_parquet(OUTPUT_PARQUET, engine="pyarrow", index=False)
    log.info(f"   Arquivo salvo: {OUTPUT_PARQUET}")
    ...
    if erros:
        ...
        log.error(f"   Arquivo {OUTPUT_PARQUET} DELETADO por falha na validação")
...
def etapa_gerar_parquets_derivados(df: pd.DataFrame, log: logging.Logger) -> dict:
    ...
    # ── olist_completo.parquet ──
    df.to_parquet(COMPLETO_PARQUET, engine="pyarrow", index=False)
    log.info(f"   ✅ olist_completo.parquet salvo — {df.shape[0]:,} linhas (todos os pedidos)")
