...
<style>
    ...
    [data-testid="stColumn"] [data-testid="stPlotlyChart"] > div {
        flex: 1 !important;
    }
...
#  ABAS PRINCIPAIS
# ═══════════════════════════════════════════════════════
aba1, aba2 = st.tabs(["Análise Exploratória", "Diagnóstico"])
...
with aba1:
    if not df_receita.empty and not df_completo.empty:
        ...
        col1.metric("Faturamento Total", f"R$ {receita_total/1e6:.2f}M")
        
        pedidos_unicos = df_receita['order_id'].nunique()
        col2.metric("Pedidos Faturados", f"{pedidos_unicos/1e3:.1f}k")

        ticket_medio = receita_total / pedidos_unicos
        col3.metric("Ticket Médio", f"R$ {ticket_medio:.2f}")
        
        total_abs_pedidos = df_receita['order_id'].nunique()
        pedidos_entregues = df_receita[df_receita['order_status'] == 'delivered']['order_id'].nunique()
        taxa_entrega = (pedidos_entregues / total_abs_pedidos) * 100
        col4.metric("Eficiência de Entrega", f"{taxa_entrega:.1f}%")
        ...
        with sub_aba1:
            ...
            with row1_col1:
                fig_status = render_distribuicao_status(df_receita)
                st.plotly_chart(fig_status, use_container_width=True)
                mostrar_insight_card("📊", "Taxa de Sucesso Operacional", "A operação demonstra alta maturidade logística, com a imensa maioria dos pedidos (Entregues) consolidando o fluxo financeiro de forma saudável e previsível.")
            with row1_col2:
                fig_pagamentos = render_metodos_pagamento(df_receita)
                st.plotly_chart(fig_pagamentos, use_container_width=True)
                mostrar_insight_card("💳", "Hegemonia do Crédito: 3 em cada 4 Pedidos são Financiados", "O Cartão de Crédito consolidou-se como o motor absoluto da operação, detendo 75,5% de market share interno. Este volume é quase 4x superior ao segundo colocado (Boleto), indicando que a conveniência do parcelamento e a aprovação instantânea são os pilares fundamentais para a conversão na plataforma.")
            
            st.markdown("<br>", unsafe_allow_html=True)
            fig_evolucao = render_evolucao_vendas(df_receita)
            st.plotly_chart(fig_evolucao, use_container_width=True)
            mostrar_insight_card("📈", "Crescimento Consistente e Sazonalidade", "Olist apresenta forte crescimento ano após ano (YoY). Note o pico marcante em Novembro de 2017, impulsionado agressivamente pelas campanhas de Black Friday.")
        ...
        with sub_aba2:
            ...
            with row2_col1:
                ...
                mostrar_insight_card("🛍️", "Composição do Faturamento por Categoria", "O Top 3 (Beleza, Relógios e Cama/Mesa) soma R$ 3,5M (~41%) do faturamento acumulado. Beleza e Saúde consolida-se como o principal driver financeiro, mantendo uma vantagem de R$ 300K em relação à terceira posição.")
            with row2_col2:
                fig_ticket = render_top_ticket(df_receita)
                st.plotly_chart(fig_ticket, use_container_width=True)
                mostrar_insight_card("💎", "Análise de Ticket Médio: Dominância da Categoria de Computadores", "Embora o volume total seja pulverizado em itens de baixo custo, a categoria de Computadores isola-se com um ticket médio de R$ 1.098, valor 2,3x superior ao segundo colocado. Isso indica uma oportunidade de faturamento concentrado: cada pedido de hardware equivale a ~6 pedidos de móveis de jardim.")

        with sub_aba3:
            fig_geo = render_receita_estados(df_receita)
            if fig_geo:
                st.plotly_chart(fig_geo, use_container_width=True)
                mostrar_insight_card("🗺️", "Distribuição Geográfica: O Eixo Sul-Sudeste concentra ~80% da Receita", "A região Sudeste isola-se com ~65% do faturamento total, impulsionada pela dominância de São Paulo (38,3%). Somado ao Sul, o eixo Sul-Sudeste controla ~80% do mercado nacional, evidenciando uma alta densidade de consumo e infraestrutura logística nessas regiões.")
        ...
        with sub_aba_comportamento:
            ...
            with row3_col1:
                ...
                if fig_comp:
                    ...
                    mostrar_insight_card("🕓", "Otimização Temporal: Janela de Conversão em Horário Comercial", "O volume de pedidos concentra-se entre Segunda e Sexta-feira das 10h às 16h. O desaquecimento acentuado aos finais de semana e madrugadas permite otimizar lances de mídia fora desses períodos.")
            
            with row3_col2:
                fig_recorrencia = render_recorrencia_clientes(df_receita)
                if fig_recorrencia:
                    st.plotly_chart(fig_recorrencia, use_container_width=True)
                    mostrar_insight_card("🧲", "Análise de Retenção: O Desafio da Recompra", "A taxa de retenção de ~3% evidencia um modelo de crescimento puramente transacional. Com 97% da base realizando apenas uma compra, a pressão sobre a margem para atrair novos usuários é constante. Implementar estratégias de CRM para estimular a recompra é vital para aumentar o valor de vida do cliente (LTV) e garantir a sustentabilidade do negócio.")
        
        with sub_aba4:
            row4_col1, row4_col2 = st.columns(2)
            with row4_col1:
                fig_satisfacao = render_satisfacao_cliente(df_receita)
                if fig_satisfacao:
                    st.plotly_chart(fig_satisfacao, use_container_width=True)
                    mostrar_insight_card("⭐", "Polarização do Sentimento", "A grande maioria das experiências é positiva, com a nota 5 estrelas liderando o volume, seguida pela nota 4 estrelas (19,5%). Nota-se um comportamento extremista: a avaliação 1 estrela (10,7%) é individualmente mais frequente que as notas intermediárias 2 ou 3, indicando que o cliente raramente utiliza tons neutros para avaliar.")
            with row4_col2:
                fig_paradoxo = render_paradoxo_valor(df_receita)
                if fig_paradoxo:
                    st.plotly_chart(fig_paradoxo, use_container_width=True)
                    mostrar_insight_card("💸", "Ticket Médio vs. Satisfação: O Custo da Expectativa", "O ticket médio das avaliações 1 estrela (R$ 165) é ~22% superior ao das avaliações 5 estrelas. Esse dado confirma que a tolerância ao erro é menor em produtos de alto valor agregado, onde falhas operacionais ou de qualidade geram as maiores frustrações no marketplace.")
        ...
        with diag_aba1:
            ...
            with row_diag1_col1:
                ...
                if fig_churn:
                    ...
                    mostrar_insight_card("📉", "Performance de Receita: Conversão de 99,3%", "A operação retém 99,3% do faturamento potencial. O vazamento de R$ 97,2k concentra-se quase inteiramente em Cancelamentos (R$ 95,2k). O impacto por Falta de Estoque (R$ 2,0k) é residual, evidenciando boa gestão de inventário.", alerta=True)
            with row_diag1_col2:
                fig_mapa = render_mapa_churn(df_completo)
                if fig_mapa:
                    st.plotly_chart(fig_mapa, use_container_width=True)
                    mostrar_insight_card("📍", "Disparidade Regional de Churn", "A concentração de cancelamentos em estados do Norte revela um desvio crítico em relação à média nacional. Corrigir essa disparidade regional é fundamental para estabilizar o desempenho operacional e reduzir a evasão de receita nessas localidades.", alerta=True)
        
        with diag_aba2:
            # Mantendo o gráfico antigo aqui temporariamente (poderia expandir)
            ...
            if fig_culpabilidade:
                ...
                mostrar_insight_card("🚨", "Foco no Operador Logístico", "Embora haja atritos e demoras na postagem pelo próprio Lojista (Vendedor), a esmagadora mancha de falhas nos SLAs reporta-se diretamente à logística ineficiente em trânsito.", alerta=True)
                
        with diag_aba3:
            fig_pareto = render_pareto_detratoras(df_receita)
            if fig_pareto:
                st.plotly_chart(fig_pareto, use_container_width=True)
                mostrar_insight_card("📦", "Concentração Crítica de Insatisfação", "A categoria de Cama, Mesa e Banho lidera isoladamente o ranking de avaliações negativas na plataforma. Essa predominância de detratores sinaliza um gargalo de qualidade específico no segmento e exige auditoria prioritária para preservar a reputação do catálogo.", alerta=True)
